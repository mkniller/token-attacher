...
(async () => {
  ...
  const localizedStrings = {
    error: {
      ...
      ElementAlreadyAttachedInChain:
        "TOKENATTACHER.error.ElementAlreadyAttachedInChain",
      ActorDataModelNeedsMigration:
        "TOKENATTACHER.error.ActorDataModelNeedsMigration",
      MigrationErrorScene: "TOKENATTACHER.error.MigrationErrorScene",
      QuickEditNotFinished: "TOKENATTACHER.error.QuickEditNotFinished",
      PostProcessingNotFinished:
        "TOKENATTACHER.error.PostProcessingNotFinished",
    ...
    info: {
      ...
      ObjectsCanNotMoveConstrained:
        "TOKENATTACHER.info.ObjectsCanNotMoveConstrained",
    ...
    button: {
      ...
      gmMenu: {
        ...
        CompendiumsMigration:
          "TOKENATTACHER.button.gmMenu.CompendiumsMigration",
  ...
  class TASettings extends FormApplication {
    ...
    activateListeners(html) {
      ...
      reset_migration.click(() => {
        TokenAttacher._resetMigration();
      });
      force_scene_migration.click(() => {
        TokenAttacher._migrateScene();
      });
      force_actor_migration.click(() => {
        TokenAttacher.migrateAllPrototypeActors();
      });
      force_compendium_migration.click(() => {
        TokenAttacher.migrateAllActorCompendiums();
      });
      import_json_dialog.click(() => {
        TokenAttacher.importFromJSONDialog();
      });
      export_actors_to_json.click(() => {
        TokenAttacher.getActorsWithPrototype();
      });
      purge_ta_data_in_scene.click(() => {
        TokenAttacher.purgeTAData();
      });
    }
  }
  ...
  class TokenAttacher {
    ...
    static initMacroAPI() {
      if (
        foundry.utils.getProperty(window, "tokenAttacher.attachElementToToken")
      )
        return;
      window.tokenAttacher = {
        ...
        getAllAttachedElementsOfToken:
          TokenAttacher.getAllAttachedElementsOfToken,
        getAllAttachedElementsByTypeOfToken:
          TokenAttacher.getAllAttachedElementsByTypeOfToken,
        getActorsWithPrototype: TokenAttacher.getActorsWithPrototype,
        getActorsWithPrototypeInCompendiums:
        ...
        setElementsMoveConstrainedStatus:
          TokenAttacher.setElementsMoveConstrainedStatus,
        ...
        _compatiblity: {
          registerLayerByDocumentName:
            TokenAttacher._registerLayerByDocumentName,
    ...
    static _registerLayerByDocumentName(type) {
      ...
      Hooks.on(`update${type}`, (document, change, options, userId) =>
        TokenAttacher.updateOffset(type, document, change, options, userId)
      );
      Hooks.on(`preUpdate${type}`, (document, change, options, userId) =>
        TokenAttacher.isAllowedToMove(type, document, change, options, userId)
      );
      Hooks.on(`preUpdate${type}`, (document, change, options, userId) =>
        TokenAttacher.handleBaseMoved(document, change, options, userId)
      );
      Hooks.on(`preUpdate${type}`, (document, change, options, userId) =>
        TokenAttacher.doAttachmentsNeedUpdate(document, change, options, userId)
      );
      Hooks.on(`preDelete${type}`, (document, options, userId) =>
        TokenAttacher.isAllowedToMove(type, document, {}, options, userId)
      );
      Hooks.on(`control${type}`, (object, isControlled) =>
        TokenAttacher.isAllowedToControl(object, isControlled)
      ); //Check hook signature
      //Deleting attached elements should detach them
      Hooks.on(`delete${type}`, (document, options, userId) =>
        TokenAttacher.DetachAfterDelete(type, document, options, userId)
      );
      ...
      Hooks.on(`create${type}`, (document, options, userId) =>
        TokenAttacher.ReattachAfterUndo(type, document, options, userId)
      );
      //Instant Attach on create if UI is open
      Hooks.on(`preCreate${type}`, (document, data, options, userId) =>
        TokenAttacher.PreInstantAttach(type, document, data, options, userId)
      );
      Hooks.on(`create${type}`, (document, options, userId) =>
        TokenAttacher.InstantAttach(type, document, options, userId)
      );
    }
    ...
    static registerHooks() {
      ...
      Hooks.on("getSceneControlButtons", (controls) =>
        TokenAttacher._getControlButtons(controls)
      );
      Hooks.on("canvasReady", () => TokenAttacher.initialize());
      Hooks.once("ready", () => {
        game.socket.on(`module.${moduleName}`, (message) =>
          TokenAttacher.listen(message)
        );
      });
      ...
      Hooks.on("canvasReady", () => {
        libWrapper.register(
          moduleName,
          "canvas.mouseInteractionManager.callbacks.dragLeftDrop",
      ...
      Hooks.on("preUpdateToken", (document, change, options, userId) =>
        TokenAttacher.UpdateBasePosition(
          "Token",
          document,
          change,
          options,
      ...
      Hooks.on("updateToken", (document, change, options, userId) =>
        TokenAttacher.UpdateAttachedOfToken(
          "Token",
          document,
          change,
          options,
      ...
      Hooks.on("updateActor", (document, change, options, userId) =>
        TokenAttacher.updateAttachedPrototype(document, change, options, userId)
      );
      Hooks.on("preCreateToken", (document, data, options, userId) =>
        TokenAttacher.preCreateBase(document, data, options, userId)
      );
      Hooks.on("createToken", (document, options, userId) =>
        TokenAttacher.updateAttachedCreatedToken(
          "Token",
          document,
          options,
          userId
      ...
      Hooks.on("pasteToken", (copy, toCreate) =>
        TokenAttacher.pasteTokens(copy, toCreate)
      );
      Hooks.on("deleteToken", (document, options, userId) =>
        TokenAttacher.deleteToken(document, options, userId)
      );
      Hooks.on("canvasInit", (canvasObj) =>
        TokenAttacher.canvasInit(canvasObj)
      );
      Hooks.on(
        "createPlaceableObjects",
        (parent, createdObjs, options, userId) =>
          TokenAttacher.batchPostProcess(parent, createdObjs, options, userId)
      );
          ...
          callback: (target) => {
            ...
            if (pack.metadata.type !== "Actor")
              return ui.notifications.error(
                game.i18n.format(localizedStrings.error.ExportAllowsOnlyActor)
              );
    ...
    static async startMigration() {
      let currentDataModelVersion = game.settings.get(
        moduleName,
        "data-model-version"
      );
    ...
    static async migrateToDataModel_3() {
      ui.notifications.info(
        game.i18n.format(localizedStrings.info.MigrationInProgress, {
          version: dataModelVersion,
        })
      );
      let scene_id_array = [];
      let scene_collection = game.scenes;
      ...
      try {
        if (remaining_scenes.length > 0) {
          if (game.scenes.get(remaining_scenes[0]) !== game.scenes.active) {
            Hooks.once("canvasReady", () =>
              TokenAttacher.migrateSceneHook(remaining_scenes)
            );
            return;
          } else {
            for (const token of canvas.tokens.placeables) {
              const attached =
                token.document.getFlag(moduleName, "attached") || {};
              if (Object.keys(attached).length > 0) {
                await TokenAttacher._attachElementsToToken(
                  attached,
                  token,
                  true
                );
            ...
            console.log(
              "Token Attacher | " +
                game.i18n.format(localizedStrings.info.MigratedScene, {
                  scenename: game.scenes.active.name,
                })
            );
            ui.notifications.info(
              game.i18n.format(localizedStrings.info.MigratedScene, {
        ...
        console.log(
          "Token Attacher | " +
            game.i18n.format(localizedStrings.info.DataModelMergedTo, {
              version: dataModelVersion,
            })
        );
        ui.notifications.info(
        ...
        ui.notifications.error(
          game.i18n.format(localizedStrings.error.MigrationErrorScene, {
      ...
      const allActors = [...game.actors].filter((actor) => {
        const attached =
          foundry.utils.getProperty(
            actor,
            `prototypeToken.flags.${moduleName}.prototypeAttached`
          ) || {};
        if (Object.keys(attached).length > 0) return true;
        return false;
      });
      const allMappedActors = allActors.map(async (actor) => {
        return await TokenAttacher.migrateActor(actor);
      });

      console.log(
        "Token Attacher | " +
          game.i18n.format(localizedStrings.info.MigratedActors)
      );
      ui.notifications.info(
        game.i18n.format(localizedStrings.info.MigratedActors)
      );
    ...
    static async migrateActor(actor, return_data = false) {
      let tokenData = await TokenAttacher.migrateElement(
        null,
        null,
        foundry.utils.duplicate(
          foundry.utils.getProperty(actor, `prototypeToken`)
        ),
        "Token"
      );
      foundry.utils.setProperty(
        tokenData,
        `flags.${moduleName}.grid`,
        TokenAttacher.getCurrentGrid()
      );
      ...
      switch (model) {
        case 2:
          return prototypeAttached[
            Object.keys(prototypeAttached)[0]
          ].hasOwnProperty("objs");
      }
      return false;
    }

    static async migrateElement(
      parent_data,
      parent_type,
      objData,
      type,
      ...
      //Migrate to offset
      if (parent_data) {
        const offset = foundry.utils.getProperty(
          objData,
          `flags.${moduleName}.offset`
        );
        if (!offset) {
          let parent_pos = foundry.utils.duplicate(
            foundry.utils.getProperty(parent_data, `flags.${moduleName}.pos`)
          );
          foundry.utils.setProperty(
            objData,
            `flags.${moduleName}.parent`,
            parent_pos.base_id
          );
          foundry.utils.setProperty(
            objData,
            `flags.${moduleName}.offset`,
            TokenAttacher.getElementOffset(
              type,
              objData,
              parent_type,
              foundry.utils.mergeObject(
                foundry.utils.mergeObject(parent_pos, parent_data),
                parent_pos.xy
              ),
              {}
            )
          ...
          let migrated_offset = TokenAttacher.getElementOffset(
            type,
            objData,
            parent_type,
            foundry.utils.mergeObject(
          ...
          foundry.utils.setProperty(
            objData,
            `flags.${moduleName}.offset`,
            foundry.utils.mergeObject(migrated_offset, offset)
          );
        }
      }
      //Migrate Attached
      const prototypeAttached = foundry.utils.getProperty(
        objData,
        `flags.${moduleName}.prototypeAttached`
      );
      if (prototypeAttached) {
        if (TokenAttacher.isPrototypeAttachedModel(prototypeAttached, 2)) {
          //Set Pos
          let posData = foundry.utils.getProperty(
            objData,
            `flags.${moduleName}.pos`
          );
    ...
    static async migrateAllActorCompendiums() {
      ...
      for (let i = 0; i < allCompendiums.length; i++) {
        ...
        console.log(
          "Token Attacher | " +
            game.i18n.format(localizedStrings.info.MigratingCompendium, {
        ...
        for (const index of packIndex) {
          ...
          if (prototypeAttached) {
            if (TokenAttacher.isPrototypeAttachedModel(prototypeAttached, 2)) {
              ...
              await pack.updateEntity(index._id, { prototypeToken: update });
            }
          }
        }
      }
      console.log(
        "Token Attacher | " +
          game.i18n.format(localizedStrings.info.MigratedCompendiums)
      );
      ui.notifications.info(
        game.i18n.format(localizedStrings.info.MigratedCompendiums)
      );
    ...
    static UpdateBasePosition(type, document, change, options, userId) {
      ...
      if (
        game.users.find((u) => u._id == userId)?.viewedScene !=
    ...
    static async UpdateAttachedOfToken(
      type,
      document,
      change,
      options,
      ...
      if (
        !foundry.utils.getProperty(
          options,
          `${moduleName}.attachmentsNeedUpdate`
        )
      )
        return;
      ...
      if (game.userId === userId && game.user.isGM) {
        let quickEdit = foundry.utils.getProperty(
          window,
          "tokenAttacher.quickEdit"
        );
        if (quickEdit && canvas.scene._id === quickEdit.scene) {
          if (!foundry.utils.getProperty(options, `${moduleName}.QuickEdit`))
            return;
          clearTimeout(quickEdit.timer);
          TokenAttacher._quickEditUpdateOffsetsOfBase(
            quickEdit,
            type,
            document
          );
          quickEdit.timer = setTimeout(
            TokenAttacher.saveAllQuickEditOffsets,
            1000
          );
          return;
        }
      ...
      const eventdata = [
        type,
        foundry.utils.mergeObject(
          foundry.utils.duplicate(base.document),
          change
        ),
      ];
      if (TokenAttacher.isFirstActiveGM())
        return TokenAttacher._UpdateAttachedOfBase(...eventdata);
      else
        return game.socket.emit(`module.${moduleName}`, {
          event: `_UpdateAttachedOfBase`,
          eventdata: eventdata,
        });
    }
    ...
    static async _UpdateAttachedOfBase(type, baseData, return_data = false) {
      const attached =
        foundry.utils.getProperty(baseData, `flags.${moduleName}.attached`) ||
        {};
      let attachedEntities = {};

      //Get Entities
      for (const key in attached) {
        attachedEntities[key] = attached[key].map((id) =>
          TokenAttacher.layerGetElement(key, id)
        );
        //Filter null entities incase an invalid id was looked up
        attachedEntities[key] = attachedEntities[key].filter(
          (entity) => entity != null
        );
      }
      ...
      for (const key in attachedEntities) {
        if (attachedEntities.hasOwnProperty(key)) {
          ...
          updates[key] = await TokenAttacher.offsetPositionOfElements(
            key,
            attachedEntities[key].map((entity) =>
              foundry.utils.duplicate(entity.document)
            ),
            type,
            baseData,
            {}
        ...
        if (attachedEntities.hasOwnProperty(key)) {
          for (let i = 0; i < attachedEntities[key].length; i++) {
            ...
            const elem_attached =
              foundry.utils.getProperty(
                element.document,
                `flags.${moduleName}.attached`
              ) || {};
            if (Object.keys(elem_attached).length > 0) {
              const elem_update = updates[key].find(
                (item) => item._id === elem_id
              );
              const updatedElementData = foundry.utils.mergeObject(
                foundry.utils.duplicate(element.document),
                elem_update
              );
              ...
              for (const key in subUpdates) {
                if (subUpdates.hasOwnProperty(key)) {
                  ...
                  let base_updates = updates[key].filter(
                    (item) => item._id === elem_id
                  );
                  if (base_updates.length > 0) {
                    let non_base_updates = updates[key].filter(
                      (item) => item._id !== elem_id
                    );
                    let merge_update = {};
                    for (let j = 0; j < base_updates.length; j++) {
                      merge_update = foundry.utils.mergeObject(
                        merge_update,
                        base_updates[j]
                      );
                    }
      ...
      for (const key in updates) {
        await TokenAttacher.safeUpdateEmbeddedDocuments(key, updates[key], {
    ...
    static getBasePositon(type, base, overrideData) {
      ...
      if (overrideData)
        objData = foundry.utils.mergeObject(objData, overrideData);

      pos = {
        base_id: foundry.utils.getProperty(objData, "_id"),
        ...
        elevation:
          objData.elevation ??
          objData.flags?.levels?.rangeBottom ??
          objData.flags?.wallHeight?.wallHeightBottom ??
          objData.flags?.["wall-height"]?.bottom,
      };

      let objSizeData = foundry.utils.duplicate(base.document ?? base);
      let validKeys = ["width", "radius", "distance", "config", "height"];
      Object.keys(objSizeData).forEach((key) => {
        validKeys.includes(key) || delete objSizeData[key];
      });
      if (objSizeData.config) {
        validKeys = ["dim", "bright"];
    ...
    static saveBasePositon(type, base, return_data = false, overrideData) {
      ...
      return { _id: objData._id, [`flags.${moduleName}.pos`]: pos };
    }

    static offsetPositionOfElements(
      type,
      objData,
      baseType,
      baseData,
      grid,
      grid_multi = TokenAttacher.getDefaultGridMultiplier()
    ) {
      let baseOffset = {};
      baseOffset.center = TokenAttacher.getCenter(baseType, baseData, grid);
      baseOffset.rotation =
        foundry.utils.getProperty(baseData, "rotation") ??
        foundry.utils.getProperty(baseData, "direction");
      baseOffset.size = TokenAttacher.getSize(baseData);
      baseOffset.elevation =
        baseData.elevation ??
        baseData.flags?.levels?.elevation ??
        baseData.flags?.levels?.rangeBottom ??
        baseData.flags?.wallHeight?.wallHeightBottom ??
        baseData.flags?.["wall-height"]?.bottom;
      ...
      let updates = objData.map((document) => {
        return foundry.utils.mergeObject(
          { _id: document._id },
          TokenAttacher.offsetPositionOfElement(
            type,
            document,
            baseType,
            baseData,
    ...
    static offsetPositionOfElement(
      type,
      objData,
      baseType,
      baseData,
      ...
      foundry.utils.setProperty(
        objData,
        `flags.${moduleName}.offset`,
        TokenAttacher.updateOffsetWithGridMultiplicator(
          type,
          objData.flags[moduleName].offset,
          grid_multi
        )
      );
      const offset = foundry.utils.getProperty(
      ...
      const size_multi = {
        w: baseOffset.size[0] / offset.size.widthBase,
        h: baseOffset.size[1] / offset.size.heightBase,
      };
      let update = {};
      ...
      if ("c" in objData) {
        ...
        [c[0], c[1]] = TokenAttacher.moveRotatePoint(
          offset,
          baseOffset.center,
          baseOffset.rotation,
          size_multi
        ...
        [c[2], c[3]] = TokenAttacher.moveRotatePoint(
          offset,
          baseOffset.center,
          baseOffset.rotation,
          size_multi
      ...
      //Rectangle Entities
        ...
        ("shape" in objData && "width" in objData.shape) ||
        "width" in objData ||
        "distance" in objData ||
        "dim" in objData ||
        ...
        const [x, y, rotation] = TokenAttacher.moveRotateRectangle(
          objData,
          offset,
          baseOffset.center,
          baseOffset.rotation,
        ...
        if (
          objData.shape?.hasOwnProperty("width") &&
          objData.shape.width != null
        ) {
        ...
        if (
          objData.hasOwnProperty("config") &&
          objData.config.hasOwnProperty("dim")
        ) {
          update.config = {};
          update.config.dim =
            (offset.size.config?.dim ?? offset.size.dim) * size_multi.w;
          update.config.bright =
            (offset.size.config?.bright ?? offset.size.bright) * size_multi.w;
        }
      ...
      //Point Entities
        ...
        const [x, y] = TokenAttacher.moveRotatePoint(
          offset,
          baseOffset.center,
          baseOffset.rotation,
          size_multi
      ...
      //Other Modules
      Hooks.callAll(
        `${moduleName}.offsetPositionOfElement`,
        type,
        objData,
        baseType,
        baseData,
    ...
    static computeRotatedPosition(x, y, x2, y2, rotRad, size_multi) {
      ...
      return [
        x + Math.cos(rotRad) * dx - Math.sin(rotRad) * dy,
    ...
    static moveRotateRectangle(
      ...
      [rectCenter.x, rectCenter.y] = TokenAttacher.computeRotatedPosition(
        anchorCenter.x,
        anchorCenter.y,
        rectCenter.x,
        rectCenter.y,
    ...
    static moveRotatePoint(offset, anchorCenter, anchorRot, size_multi) {
      ...
      // rotate vector around angle
      [point.x, point.y] = TokenAttacher.computeRotatedPosition(
        anchorCenter.x,
        anchorCenter.y,
        point.x,
        point.y,
    ...
    static isFirstActiveGM() {
      ...
      const firstGm = game.users.find(
        (u) => u.isGM && u.active && u.viewedScene === currentScene
      );
    ...
    static detectGM() {
      ...
      if (!firstGm) {
        return ui.notifications.error(
          game.i18n.format(localizedStrings.error.NoActiveGMFound)
        );
      }
    }
    ...
    static async workListenQueue() {
      ...
      switch (message.event) {
        ...
        case "AttachToToken":
          if (TokenAttacher.isFirstActiveGM())
            result = TokenAttacher._AttachToToken(...message.eventdata);
          break;
        case "DetachFromToken":
          if (TokenAttacher.isFirstActiveGM())
            result = TokenAttacher._DetachFromToken(...message.eventdata);
          break;
        case "attachElementsToToken":
          if (TokenAttacher.isFirstActiveGM())
            result = TokenAttacher._attachElementsToToken(...message.eventdata);
          break;
        case "detachElementsFromToken":
          if (TokenAttacher.isFirstActiveGM())
            result = TokenAttacher._detachElementsFromToken(
              ...message.eventdata
            );
          break;
        case "ReattachAfterUndo":
          if (TokenAttacher.isFirstActiveGM())
            result = TokenAttacher._ReattachAfterUndo(...message.eventdata);
          break;
        case "UpdateAttachedOfBase":
          if (TokenAttacher.isFirstActiveGM())
            result = TokenAttacher._UpdateAttachedOfBase(...message.eventdata);
          break;
        case "setElementsLockStatus":
          if (TokenAttacher.isFirstActiveGM())
            result = TokenAttacher._setElementsLockStatus(...message.eventdata);
          break;
        case "setElementsMoveConstrainedStatus":
          if (TokenAttacher.isFirstActiveGM())
            result = TokenAttacher._setElementsMoveConstrainedStatus(
              ...message.eventdata
            );
          break;
        default:
    ...
    static async _AttachToToken(
      ...
      if (!token)
        return ui.notifications.error(
          game.i18n.format(localizedStrings.error.NoTokensSelected)
        );
      if (!elements.hasOwnProperty("type")) return;

      let updates = {};
      let attached = foundry.utils.duplicate(
        token.document.getFlag(moduleName, `attached.${elements.type}`) || []
      );

      attached = attached.concat(
        elements.ids.filter((item) => attached.indexOf(item) < 0)
      );
      //Filter non existing
      attached = attached.filter((item) =>
        TokenAttacher.layerGetElement(elements.type, item)
      );
      let all_attached = foundry.utils.duplicate(
        token.document.getFlag(moduleName, `attached`) || {}
      );
      ...
      if (dup !== false) {
        console.log(
          "Token Attacher | You tried to attach an Element that is already attached somwhere in the chain: ",
          dup
        );
        return ui.notifications.error(
          game.i18n.format(localizedStrings.error.ElementAlreadyAttachedInChain)
        );
      }

      let token_update = await TokenAttacher.saveBasePositon(
        token.layer.constructor.documentName,
        token,
        true
      );
      ...
      for (let i = 0; i < attached.length; i++) {
        const element = TokenAttacher.layerGetElement(
          elements.type,
          attached[i]
        );
        updates[elements.type].push({
          _id: attached[i],
          [`flags.${moduleName}.parent`]: token.document._id,
          [`flags.${moduleName}.offset`]: TokenAttacher.getElementOffset(
            elements.type,
            element.document,
            token.layer.constructor.documentName,
            token.document,
      ...
      if (token.layer.constructor.documentName !== elements.type) {
        await TokenAttacher.safeUpdateEmbeddedDocuments(
          token.layer.constructor.documentName,
          updates[token.layer.constructor.documentName]
        );
      }
      await TokenAttacher.safeUpdateEmbeddedDocuments(
        elements.type,
        updates[elements.type]
      );

      if (!suppressNotification)
        ui.notifications.info(
          game.i18n.format(localizedStrings.info.ObjectsAttached)
        );
      return;
    }
    ...
    static async _updateAttachedOffsets({ type, element }) {
      const updateFunc = async (base) => {
        ...
        for (const key in attached) {
          if (attached.hasOwnProperty(key) && key !== "unknown") {
            await TokenAttacher._AttachToToken(
              base,
              { type: key, ids: attached[key] },
              true
            );
            await TokenAttacher._updateAttachedOffsets({
              type: key,
              element: attached[key],
            });
          }
        }
      };
      if (typeof element === "string" || element instanceof String)
        element = TokenAttacher.layerGetElement(type, element);
    ...
    static async _DetachFromToken(
      ...
      if (!elements || !elements.hasOwnProperty("type")) {
        ...
        if (Object.keys(attached).length > 0) {
          for (const key in attached) {
            if (attached.hasOwnProperty(key)) {
              ...
              if (deletes.length > 0)
                await TokenAttacher.safeUpdateEmbeddedDocuments(key, deletes, {
                  [moduleName]: { update: true },
                });
            }
          }
        ...
        if (!suppressNotification)
          ui.notifications.info(
            game.i18n.format(localizedStrings.info.ObjectsDetached)
          );
        return;
      } else {
        ...
        for (let i = 0; i < elements.ids.length; i++) {
          deletes.push({
            _id: elements.ids[i],
            [`flags.-=${moduleName}`]: null,
          });
        }
        if (deletes.length > 0 && !options.skip_update)
          await TokenAttacher.safeUpdateEmbeddedDocuments(elements.type, deletes, {
            [moduleName]: { update: true },
          });
        await token.document.setFlag(
          moduleName,
          `attached.${elements.type}`,
          attached
        );
        if (!suppressNotification)
          ui.notifications.info(
      ...
      for (let i = 0; i < controls.length; i++) {
        if (controls[i].name === "token") {
          controls[i].tools.push({
            ...
            onClick: () =>
              TokenAttacher._StartTokenAttach(canvas.tokens.controlled[0]),
            button: true,
          });
          controls[i].tools.push({
            name: "TAToggleQuickEdit",
            title: game.i18n.format(
              localizedStrings.button.ToggleQuickEditMode
            ),
            ...
            toggle: true,
            active:
              foundry.utils.getProperty(window, "tokenAttacher.quickEdit") ??
              false,
          });
        }
      }
    ...
    static async attachElementToToken(
      element,
      target_token,
      suppressNotification = false
    ) {
      ...
      if (TokenAttacher.isFirstActiveGM())
        return await TokenAttacher._AttachToToken(
          target_token,
          { type: type, ids: selected },
    ...
    static async attachElementsToToken(
      element_array,
      target_token,
      suppressNotification = false
    ) {
      ...
      if (TokenAttacher.isFirstActiveGM())
        return await TokenAttacher._attachElementsToToken(
          selected,
          target_token,
          suppressNotification
        );
    ...
    static async _attachElementsToToken(
      selected,
      target_token,
      suppressNotification = false
    ) {
      if (typeof target_token === "string" || target_token instanceof String)
        target_token = canvas.tokens.get(target_token);
      let updates = {};
      const type = target_token.layer.constructor.documentName;
      for (const key in selected) {
        if (selected.hasOwnProperty(key)) {
          let newUpdates = await TokenAttacher._AttachToToken(
            target_token,
            { type: key, ids: selected[key] },
            suppressNotification,
            true
          );
          if (Object.keys(updates).length <= 0 && newUpdates)
            updates = newUpdates;
          else {
      ...
      if (updates.hasOwnProperty(type)) {
        let target_token_updates = updates[type].filter(
          (item) => item._id === target_token.document._id
        );
        let other_updates = updates[type].filter(
          (item) => item._id !== target_token.document._id
        );
        let base_updates = {};
        for (let i = 0; i < target_token_updates.length; i++) {
          base_updates = foundry.utils.mergeObject(
            base_updates,
            target_token_updates[i]
          );
        }
    ...
    static async showTokenAttacherUI(token) {
      ...
      if (TokenAttacher.isAttachmentUIOpen())
        await TokenAttacher.closeTokenAttacherUI();
      await canvas.scene.setFlag(moduleName, "attach_base", {
        type: token.layer.constructor.documentName,
        element: token.document._id,
      });
      const locked_status =
        token.document.getFlag(moduleName, "locked") || false;
      // Get the handlebars output
      const myHtml = await renderTemplate(
        `${templatePath}/tokenAttacherUI.html`,
        {
          ["token-image"]: token.document.texture.src,
      ...
      window.document
        .getElementById("hud")
        .insertAdjacentHTML("afterend", myHtml);
      ...
      let toggle_animate_tool =
        attachmentUI.getElementsByClassName("toggle-animate")[0];

      const base_exists = () => {
        const attachment_base = canvas.scene.getFlag(moduleName, "attach_base");
        let base;
        if (attachment_base) {
          base = this.layerGetElement(
            attachment_base.type,
            attachment_base.element
          );
        }
        if (!base) {
          TokenAttacher.closeTokenAttacherUI();
          ui.notifications.error(
            game.i18n.format(localizedStrings.error.BaseDoesntExist)
          );
          return false;
        }
        return true;
      ...
      $(link_tool).click(() => {
        ...
        if (current_layer.controlled.length <= 0)
          return ui.notifications.error(
            game.i18n.format(localizedStrings.error.NothingSelected)
          );
        if (current_layer.controlled.length == 1)
          TokenAttacher.attachElementToToken(
            current_layer.controlled[0],
            token
          );
        else {
      ...
      $(unlink_tool).click(() => {
        ...
        if (current_layer.controlled.length == 1)
          TokenAttacher.detachElementFromToken(
            current_layer.controlled[0],
            token
          );
        else {
          TokenAttacher.detachElementsFromToken(
            current_layer.controlled,
            token
          );
        }
      });
      ...
      $(select_tool).click(() => {
        ...
        if ($(attachmentUI).find(".control-tool.select.active").length > 0) {
          ui.notifications.info(
            game.i18n.format(localizedStrings.info.DragSelectElements)
          );
        }
      });
      ...
      $(toggle_animate_tool).click(() => {
        ...
        if (current_layer !== canvas.getLayerByEmbeddedName("Token"))
          return ui.notifications.error(
            game.i18n.format(localizedStrings.error.OnlyTokenToggleAnimate)
          );
    ...
    static async setElementsMoveConstrainedStatus(
      elements,
      canMoveConstrained,
      suppressNotification = false,
      options = {}
      ...
      options = foundry.utils.mergeObject(
        { type: TokenAttacher.CONSTRAINED_TYPE.TOKEN_CONSTRAINED },
        options,
        {
          insertKeys: true,
          insertValues: true,
      ...
      if (TokenAttacher.isFirstActiveGM())
        return await TokenAttacher._setElementsMoveConstrainedStatus(
          selected,
          canMoveConstrained,
          suppressNotification,
          options
    ...
    static async _setElementsMoveConstrainedStatus(
      elements,
      canMoveConstrained,
      suppressNotification,
      options
        ...
        if (elements.hasOwnProperty(key)) {
          for (let i = 0; i < elements[key].length; i++) {
            ...
            if (
              ...
              if (!updates.hasOwnProperty(key)) updates[key] = [];
              if (canMoveConstrained)
                updates[key].push({
                  _id: element.document._id,
                  [`flags.${moduleName}.canMoveConstrained`]: options,
                });
              else
                updates[key].push({
                  _id: element.document._id,
                  [`flags.${moduleName}.-=canMoveConstrained`]: null,
                });
            }
          }
        }
      ...
      if (!suppressNotification) {
        if (!canMoveConstrained)
          ui.notifications.info(
            game.i18n.format(localizedStrings.info.ObjectsCanMoveConstrained)
          );
        else
          ui.notifications.info(
            game.i18n.format(localizedStrings.info.ObjectsCanNotMoveConstrained)
          );
      }
    }

    static async setElementsLockStatus(
      elements,
      isLocked,
      suppressNotification = false
    ) {
      ...
      if (TokenAttacher.isFirstActiveGM())
        return await TokenAttacher._setElementsLockStatus(
          selected,
          isLocked,
          suppressNotification
        );
        ...
        game.socket.emit(`module.${moduleName}`, {
          ...
          eventdata: [selected, isLocked, suppressNotification],
        });
    }

    static async _setElementsLockStatus(
      elements,
      isLocked,
      suppressNotification
    ) {
          ...
          for (let i = 0; i < elements[key].length; i++) {
            ...
            if (
              ...
              if (!isLocked)
                updates[key].push({
                  _id: element.document._id,
                  [`flags.${moduleName}.unlocked`]: true,
                });
              else
                updates[key].push({
                  _id: element.document._id,
                  [`flags.${moduleName}.-=unlocked`]: null,
                });
            }
          }
        }
      ...
      if (!suppressNotification) {
        if (!isLocked)
          ui.notifications.info(
            game.i18n.format(localizedStrings.info.ObjectsUnlocked)
          );
        else
          ui.notifications.info(
            game.i18n.format(localizedStrings.info.ObjectsLocked)
          );
      }
    }
    ...
    static async detachElementFromToken(
      element,
      target_token,
      suppressNotification = false
    ) {
      ...
      if (TokenAttacher.isFirstActiveGM())
        return await TokenAttacher._DetachFromToken(
          target_token,
          { type: type, ids: selected },
          suppressNotification
        );
    ...
    static async detachElementsFromToken(
      element_array,
      target_token,
      suppressNotification = false
    ) {
      ...
      if (TokenAttacher.isFirstActiveGM())
        return await TokenAttacher._detachElementsFromToken(
          selected,
          target_token,
          suppressNotification
        );
    ...
    static async _detachElementsFromToken(
      selected,
      target_token,
      suppressNotification = false
    ) {
    ...
    static detachAllElementsFromToken(
      target_token,
      suppressNotification = false
    ) {
      if (TokenAttacher.isFirstActiveGM())
        ...
        game.socket.emit(`module.${moduleName}`, {
          event: `DetachFromToken`,
          eventdata: [target_token.document._id, {}, suppressNotification],
        });
    }

    static getAllAttachedElementsOfToken(
      target_token,
      suppressNotification = false
    ) {
    ...
    static getAllAttachedElementsByTypeOfToken(
      target_token,
      type,
      suppressNotification = false
    ) {
      return (
        target_token.document.getFlag(moduleName, `attached.${type}`) || {}
      );
    }
    ...
    static getElementOffset(type, objData, base_type, baseDoc, grid) {
      ...
      let offset = {
        ...
        rot: Number.MAX_SAFE_INTEGER,
      };
      offset.x =
        objData.x ??
        (objData.c?.[0] < objData.c?.[2] ? objData.c?.[0] : objData.c?.[2]);
      offset.y =
        objData.y ??
        (objData.c?.[1] < objData.c?.[3] ? objData.c?.[1] : objData.c?.[3]);
      ...
      let base_elevation =
        baseDoc.elevation?.bottom ??
        baseDoc.elevation ??
        baseDoc.flags["levels"]?.elevation ??
        baseDoc.flags["levels"]?.rangeBottom ??
        baseDoc.flags["wallHeight"]?.wallHeightBottom ??
        baseDoc.flags["wall-height"]?.bottom ??
        0;
      offset.elevation = {};
      ...
      if (objData.hasOwnProperty("elevation")) {
        ...
        if (
          [null, Infinity, -Infinity].includes(offset.elevation.elevation) ===
          false
        )
          offset.elevation.elevation -= base_elevation;
      }
      [offset.size.widthBase, offset.size.heightBase] =
        TokenAttacher.getSize(baseDoc);

      Hooks.callAll(
        `${moduleName}.getElementOffset`,
        type,
        objData,
        base_type,
        baseDoc,
    ...
    static updateOffsetWithGridMultiplicator(type, offset, grid_multi) {
      ...
      //Other Modules
      Hooks.callAll(
        `${moduleName}.updateOffsetWithGridMultiplicator`,
        type,
        offset,
        grid_multi
      );
    ...
    static getElementSize(element) {
      let size = {};
      size.width =
        element.document.width ??
        element.document.distance ??
        element.document.config?.dim ??
        element.document.dim ??
        element.document.radius;
      size.height =
        element.document.height ??
        element.document.distance ??
        element.document.config?.dim ??
        element.document.dim ??
        element.document.radius;
    ...
    static getObjectsFromIds(base_type, base_data, type, idArray) {
      ...
      for (const elementid of idArray) {
        ...
        if (TokenAttacher.isAttachmentBase(element.document)) {
          const prototypeAttached = TokenAttacher.generatePrototypeAttached(
            element.document
          );
    ...
    static async copyAttached(token) {
      ...
      for (const key in attached) {
        if (attached.hasOwnProperty(key) && key !== "unknown") {
          copyObjects.map[key] = TokenAttacher.getObjectsFromIds(
            "Token",
            token.document,
            key,
            attached[key]
    ...
    static async pasteAttached(token) {
      const copyObjects =
        foundry.utils.duplicate(game.user.getFlag(moduleName, "copy")) || {};
      if (Object.keys(copyObjects).length == 0) return;
      await TokenAttacher.saveBasePositon(
        token.layer.constructor.documentName,
        token
      );
      ...
      for (const key in copyObjects.map) {
        if (copyObjects.map.hasOwnProperty(key) && key !== "unknown") {
          for (let i = 0; i < copyObjects.map[key].length; i++) {
            copyObjects.map[key][i].flags[moduleName].parent =
              token.document._id;
          }
        }
      }
      ...
      await TokenAttacher.regenerateAttachedFromPrototype(
        token.layer.constructor.documentName,
        token,
        copyObjects.map,
        grid_multi,
    ...
    static async updateAttachedPrototype(document, change, options, userId) {
      ...
      if (TokenAttacher.isAttachmentUIOpen()) {
        console.log(
          "Token Attacher | " +
            game.i18n.format(localizedStrings.error.UIisOpenOnAssign)
        );
        ui.notifications.error(
          game.i18n.format(localizedStrings.error.UIisOpenOnAssign)
        );
      ...
      let prototypeAttached = TokenAttacher.generatePrototypeAttached(
    ...
    static generatePrototypeAttached(token_data, attached = {}) {
      ...
      if (Object.keys(attached).length == 0)
        attached = token_data.flags[moduleName].attached;

      for (const key in attached) {
        if (attached.hasOwnProperty(key)) {
          prototypeAttached[key] = TokenAttacher.getObjectsFromIds(
            "Token",
            token_data,
            key,
            attached[key]
    ...
    static transformBaseIntoPrototype(baseDocument) {
      ...
      const prototypeAttached =
        TokenAttacher.generatePrototypeAttached(transformedData);
    ...
    static async copyTokens(layer, tokens) {
      ...
      tokens.forEach((token) => {
        if (TokenAttacher.isAttachmentBase(token.document)) {
          prototypeMap[token.document.id] =
            TokenAttacher.generatePrototypeAttached(token.document);
        }
      });
    ...
    static pasteTokens(copy, toCreate) {
      const copyPrototypeMap =
        game.user.getFlag(moduleName, "copyPrototypeMap") || {};
      for (let i = 0; i < toCreate.length; i++) {
        if (
          toCreate[i].flags.hasOwnProperty(moduleName) &&
          ...
          if (copyPrototypeMap.map.hasOwnProperty(clsname)) {
            toCreate[i].flags[moduleName].prototypeAttached =
              copyPrototypeMap.map[clsname][copy[i].document._id];
    ...
    static async deleteToken(document, options, userId) {
      ...
      if (!TokenAttacher.isFirstActiveGM()) return;
      const attached =
        foundry.utils.getProperty(document, `flags.${moduleName}.attached`) ||
        {};
      if (Object.keys(attached).length == 0) return true;
      ...
      for (const key in deletes) {
        if (deletes.hasOwnProperty(key)) {
          ...
          await TokenAttacher.safeDeleteEmbeddedDocuments(
        ...
        if (attached.hasOwnProperty(key)) {
          ...
          for (let i = 0; i < attached[key].length; i++) {
            ...
            if (Object.keys(child_attached).length > 0) {
              const child_ids = TokenAttacher.getChildrenIds(
                child_attached,
                all_ids
              );
    ...
    static preCreateBase(document, objData, options, userId) {
      ...
      if (
        ...
        foundry.utils.setProperty(
          updates,
          `flags.${moduleName}.needsPostProcessing`,
          true
        );
      }
      if (
        foundry.utils.getProperty(document, `flags.${moduleName}.pos`) &&
        !document.flags[moduleName].pos.width
      ) {
        let pos = TokenAttacher.getBasePositon("Token", document);
        foundry.utils.setProperty(
          updates,
          `flags.${moduleName}.pos`,
          foundry.utils.mergeObject(pos, document.flags[moduleName].pos)
        );
      }
    ...
    static async updateAttachedCreatedToken(type, document, options, userId) {
      ...
      if (foundry.utils.getProperty(game, "multilevel")) {
        if (game.multilevel._isReplicatedToken(token))
          token.document.unsetFlag(moduleName, "attached");
      }
      if (
        foundry.utils.getProperty(options, "isUndo") === true &&
        foundry.utils.getProperty(options, "mlt_bypass") === true
      )
        return;
      ...
      const prototypeAttached =
        token.document.getFlag(moduleName, "prototypeAttached") || {};
      const attached = token.document.getFlag(moduleName, "attached") || {};
      ...
      if (Object.keys(prototypeAttached).length > 0) {
        if (TokenAttacher.isPrototypeAttachedModel(prototypeAttached, 2))
          return ui.notifications.error(
            game.i18n.format(
              localizedStrings.error.ActorDataModelNeedsMigration
            )
          );

        let grid_multi =
          token.document.getFlag(moduleName, "grid") ||
          TokenAttacher.getCurrentGrid();
        //Convert pre V12
    ...
    static async regenerateAttachedFromPrototype(
      baseType,
      base,
      prototypeAttached,
      grid_multi,
      ...
      grid_multi = foundry.utils.mergeObject(
      ...
      for (const key in prototypeAttached) {
        if (
          prototypeAttached.hasOwnProperty(key) &&
          key !== "unknown" &&
          prototypeAttached[key].length > 0
        ) {
          if (!toCreate.hasOwnProperty(key)) toCreate[key] = [];
          toCreate[key] = await TokenAttacher.offsetPositionOfElements(
            key,
          ...
          for (let i = 0; i < toCreate[key].length; i++) {
            ...
            entity = foundry.utils.mergeObject(
              prototypeAttached[key][i],
              entity
            );
            delete entity._id;
      ...
      for (const key in prototypeAttached) {
        for (let i = 0; i < prototypeAttached[key].length; i++) {
          ...
          const element_protoAttached = foundry.utils.getProperty(
            element,
            `flags.${moduleName}.prototypeAttached`
          );
          if (element_protoAttached) {
            const toCreateElement = toCreate[key].find(
              (item) =>
                foundry.utils.getProperty(
                  item,
                  `flags.${moduleName}.pos.base_id`
                ) ===
                foundry.utils.getProperty(
                  element,
                  `flags.${moduleName}.pos.base_id`
      ...
      if (allowed === false) {
        console.debug(
          `${moduleName} | creation of PlacableObjects prevented by preCreatePlaceableObjects hook`
        );
        return;
      }
      ...
      for (const key in toCreate) {
        if (toCreate.hasOwnProperty(key)) {
          if (key === "Tile") {
            ...
            for (let i = 0; i < toCreate[key].length; i++) {
              ...
              if (
                element.texture?.src &&
                element.texture?.src !== "" &&
                element.texture?.src !== null
              )
                promises.push(
                  loadTexture(element.texture?.src, {
          ...
          const created = await TokenAttacher.safeCreateEmbeddedDocuments(
            key,
            toCreate[key],
            options
          );
      ...
      game.socket.emit(`module.${moduleName}`, {
        ...
        eventdata: [canvas.scene, pasted, options, game.userId],
      });
      ui.notifications.info(
        game.i18n.format(localizedStrings.info.PastedAndAttached)
      );
      return;
    }
    ...
    static async regenerateLinks(createdDocs, options = {}, userId = "") {
      ...
      const pushUpdate = (key, update, updateObj) => {
        ...
        const dupIndex = updateObj[key].findIndex(
          (item) => update._id === item._id
        );
        if (dupIndex === -1) updateObj[key].push(update);
        else
          updateObj[key][dupIndex] = foundry.utils.mergeObject(
            updateObj[key][dupIndex],
            update
          );
      };
      for (const key in createdDocs) {
        if (createdDocs.hasOwnProperty(key)) {
          ...
          for (let i = 0; i < arr.length; i++) {
            const baseDoc = arr[i];
            const old_base_id = foundry.utils.getProperty(
              baseDoc,
              `flags.${moduleName}.pos.base_id`
            );
            if (old_base_id) {
              let current_attached = foundry.utils.duplicate(
                foundry.utils.getProperty(
                  baseDoc,
                  `flags.${moduleName}.attached`
                ) ?? {}
              ...
              for (const type in createdDocs) {
                if (createdDocs.hasOwnProperty(type)) {
                  new_attached[type] = createdDocs[type].filter(
                    (doc) =>
                      foundry.utils.getProperty(
                        doc,
                        `flags.${moduleName}.parent`
                  ...
                  if (
                    current_attached &&
                    current_attached.hasOwnProperty(type)
                  ) {
                    current_attached[type] = current_attached[type].filter(
                      (item) =>
                        foundry.utils.getProperty(
                          TokenAttacher.layerGetElement(type, item).document,
                          `flags.${moduleName}.parent`
                        ) === baseDoc._id
                    );
                    new_attached[type] = [
                      ...new Set(
                        new_attached[type].concat(current_attached[type])
                      ),
                    ];
                  }
              ...
              let afterUpdate = {
                ...
                [`flags.${moduleName}.pos`]: TokenAttacher.getBasePositon(
                  "Token",
                  baseDoc
                ),
              };
      ...
      if (
        foundry.utils.getProperty(
          options,
          `${moduleName}.InstantAttach.userId`
        ) === userId
      ) {
        if (foundry.utils.getProperty(options, `${moduleName}.base`)) {
          ...
          const child = foundry.utils.getProperty(
            options,
            `${moduleName}.base`
          );

          let subUpdates = await TokenAttacher._AttachToToken(
            element,
            { type: child.type, ids: [child._id] },
            true,
            true
          );
          for (const key in subUpdates) {
      ...
      for (const key in afterUpdates) {
        if (afterUpdates.hasOwnProperty(key)) {
          await TokenAttacher.safeUpdateEmbeddedDocuments(key, afterUpdates[key], {
            [moduleName]: { update: true },
          });
        }
      }
    ...
    static async deleteMissingLinks() {
      ...
      for (const type of TokenAttacher.registeredLayers) {
        const layer =
          canvas.getLayerByEmbeddedName(type) ??
          TokenAttacher.getLayerByEmbeddedName(type);
        if (!layer) continue;
        const deleteLinks = (layer) => {
          for (let i = 0; i < layer.placeables.length; i++) {
            ...
            if (
              ...
              pushUpdate(type, element.document._id, deletes);
            else if (
              foundry.utils.getProperty(
                element.document,
                `flags.${moduleName}.parent`
      ...
      for (const key in deletes) {
        if (deletes.hasOwnProperty(key)) {
          await TokenAttacher.safeDeleteEmbeddedDocuments(key, deletes[key], {
            [moduleName]: { update: true },
          });
        }
      }
    ...
    static async batchPostProcess(parent, createdDocs, options, userId) {
      ...
      if (foundry.utils.getProperty(options, `${moduleName}.base`)) {
        ...
        if (!foundry.utils.getProperty(myCreatedDocs, base.type))
          myCreatedDocs[base.type] = [base.doc];
        else {
          if (
            !myCreatedDocs[base.type].find((doc) => doc._id === base.doc._id)
          ) {
      ...
      ui.notifications.info(
        game.i18n.format(localizedStrings.info.PostProcessingFinished)
      );
    }
    ...
    static async getActorsWithPrototype() {
      ...
      const html = await renderTemplate(`${templatePath}/ImExportUI.html`, {
    ...
    static async importFromJSONDialog() {
      const html = await renderTemplate(`${templatePath}/ImExportUI.html`, {
        label_content: "Paste JSON below:",
        content: "",
      });
      Dialog.prompt({
        title: "Import Actors from JSON",
        content: html,
        callback: (html) => {
          ...
          if (!formDataObj.JSONContent) {
            const err = new Error(
              game.i18n.format(localizedStrings.error.NoValidJSONProvided)
            );
    ...
    static async importFromJSON(json, options = {}) {
      ...
      ui.notifications.info(
        game.i18n.format(localizedStrings.info.ImportingJSONStart, {
          name: name,
        })
      );
      if (imported.folder)
        await TokenAttacher.importFromJSONWithFolders(imported, options);
      if (imported.compendium)
        await TokenAttacher.importFromJSONWithCompendium(imported, options);
      console.log("Token Attacher - Finished JSON Import for " + name);
      ui.notifications.info(
        game.i18n.format(localizedStrings.info.ImportingJSONFinished, {
          name: imported.folder || imported.compendium?.name,
        })
      );
    }
    ...
    static getDefaultActorForSystem() {
      let actorType;
      let documentTypes =
        game.system.documentTypes?.Actor ?? game.system.template.Actor;
      let typesArray = Object.keys(documentTypes);
      if (game.system.id == "wfrp4e")
        actorType = typesArray.find((a) => a == "character");
          ...
          allPromises.push(
            (async (folder) => {
              ...
              const newFolder = await Folder.create(
                { name: folder.name, type: "Actor", folder: parent },
                { pack: worldCompendium.collection }
              );
              if (!parentMap.hasOwnProperty(folder._id)) {
                parentMap[folder._id] = {
                  value: new Promise((resolve) => resolve(newFolder._id)),
                };
              } else {
    ...
    static async importFromJSONWithCompendium(imported, options = {}) {
      ...
      if (options.hasOwnProperty("module-label"))
        label = "(" + options["module-label"] + ")" + label;
      let slugified_name = name.slugify({ strict: true });
      if (name !== slugified_name) {
        console.error(
          "Token Attacher - Importing a JSON Compendium where the name is not slugified, contact the author to slugify the name: ",
          label,
          name
        );
      }
      ...
      let worldCompendium = await CompendiumCollection.createCompendium({
        label: label,
        name: slugified_name,
        type: "Actor",
      ...
      // }
      return await worldCompendium.documentClass.create(creates, {
        pack: worldCompendium.collection,
      });
    }
    ...
    static isAllowedToMove(type, document, change, options, userId) {
      ...
      if (
        ...
        ui.notifications.error(
          game.i18n.format(localizedStrings.error.PostProcessingNotFinished)
        );
        return false;
      }
      ...
      let animate =
        foundry.utils.getProperty(document, `flags.${moduleName}.animate`) ??
        true;
      if (!animate) foundry.utils.setProperty(options, `animate`, animate);

      let offset =
        foundry.utils.getProperty(document, `flags.${moduleName}.offset`) || {};
      ...
      if (
        TokenAttacher.isAttachmentUIOpen() &&
        TokenAttacher.isCurrentAttachUITarget(objParent)
      )
        return true;
      if (game.user.isGM) {
        let quickEdit = foundry.utils.getProperty(
      ...
      if (
        !foundry.utils.getProperty(options, `${moduleName}.update`) &&
        foundry.utils.getProperty(
          document,
          `flags.${moduleName}.canMoveConstrained`
        )
      ) {
        const parent_token = canvas.tokens.get(objParent);
        const canMoveConstrained = foundry.utils.getProperty(
          document,
          `flags.${moduleName}.canMoveConstrained`
        );

        const updatedDocumentData = foundry.utils.mergeObject(
        ...
        switch (canMoveConstrained.type) {
          case TokenAttacher.CONSTRAINED_TYPE.TOKEN_CONSTRAINED:
            isAllowed =
              type === "Token" &&
              TokenAttacher.isMovingInParent(
                updatedDocumentData,
                parent_token.document
              );
            break;
        ...
        if (isAllowed) {
          ...
          const new_offset = TokenAttacher.getElementOffset(
            type,
            updatedDocumentData,
            base_type,
            parent_token.document,
    ...
    static isMovingInParent(child, base) {
      ...
      return (
        Number.between(
          child.x,
          base.x,
          base.x + base.width * currentGrid.sizeX
        ) &&
        Number.between(
          child.y,
          base.y,
          base.y + base.height * currentGrid.sizeY
        ) &&
        Number.between(
          child.x + child.width * currentGrid.sizeX,
          base.x,
          base.x + base.width * currentGrid.sizeX
        ) &&
        Number.between(
          child.y + child.height * currentGrid.sizeY,
          base.y,
          base.y + base.height * currentGrid.sizeY
        )
      );
    ...
    static handleBaseMoved(document, change, options, userId) {
      ...
      let attached = foundry.utils.getProperty(
        document,
        `flags.${moduleName}.attached`
      );
      if (!attached) return true;

      const mlt_block_movement =
        game.settings.get(moduleName, "MLTBlockMovement") || false;
      if (mlt_block_movement) {
        if (TokenAttacher.hasVehiclesDrawing(attached)) {
          if (foundry.utils.getProperty(options, "isUndo") === true)
            if (foundry.utils.getProperty(options, "mlt_bypass") === true)
              return false;
        }
      }
    ...
    static doAttachmentsNeedUpdate(document, change, options, userId) {
      ...
      Hooks.callAll(
        `${moduleName}.doAttachmentsNeedUpdate`,
        document,
        change,
        options,
        userId
      ...
      for (const key in attached) {
        if (attached.hasOwnProperty(key)) {
          for (let i = 0; i < attached[key].length; i++) {
            ...
            if (Object.keys(child_attached).length > 0) {
              result =
                result || TokenAttacher.hasVehiclesDrawing(child_attached);
            }
    ...
    static isCurrentAttachUITarget(id) {
      return (
        canvas.tokens.get(
          canvas.scene.getFlag(moduleName, "attach_base").element
        ).document._id === id
      );
    }
    ...
    static ReattachAfterUndo(type, document, options, userId) {
      ...
      if (foundry.utils.getProperty(options, "isUndo") === true) {
        ...
        if (TokenAttacher.isFirstActiveGM())
          TokenAttacher._ReattachAfterUndo(type, document, options, userId);
        else
          game.socket.emit(`module.${moduleName}`, {
            event: `ReattachAfterUndo`,
            eventdata: [type, document, options, userId],
          });
      }
      return;
    }
    ...
    static async _ReattachAfterUndo(type, parent, entity, options, userId) {
      let objParent =
        foundry.utils.getProperty(entity, `flags.${moduleName}.parent`) || "";
      const parent_token = canvas.tokens.get(objParent);
      if (parent_token) {
        TokenAttacher._AttachToToken(
          parent_token,
          { type: type, ids: [entity._id] },
          true
        );
      } else {
    ...
    static async _RectangleSelection(event) {
      ...
      if (
        $(window.document.getElementById("tokenAttacher")).find(
          ".control-tool.select.active"
        ).length <= 0
      )
        return;
      $(window.document.getElementById("tokenAttacher"))
        .find(".control-tool.select")[0]
      ...
      for (const type of TokenAttacher.registeredLayers) {
        ...
        const selectAll = (layer) => {
          ...
          const controllable = layer.placeables.filter(
          ...
          selected[type] = selected[type].concat(
            newSet
              .map((a) => a.document._id)
              .filter((a) => !selected[type].includes(a))
          );
          ...
          for (const elementid of attached[key]) {
            ...
            if (element) {
              const elementAttached =
                element.document.getFlag(moduleName, "attached") || {};
    ...
    static getCenter(type, objData, grid = {}) {
      grid = foundry.utils.mergeObject(
        { sizeX: canvas.grid.sizeX, sizeY: canvas.grid.sizeY },
        grid
      );
      ...
      if (type == "Token") {
        if (
          ...
          let [width, height] = [
            objData.width * grid.sizeX,
            objData.height * grid.sizeY,
          ];
          center = { x: x + Math.abs(width) / 2, y: y + Math.abs(height) / 2 };
          return center;
        }
        if (
          [CONST.GRID_TYPES.HEXODDR, CONST.GRID_TYPES.HEXEVENR].includes(
            canvas.grid.type
          )
        ) {
        ...
        if (
          [CONST.GRID_TYPES.HEXODDQ, CONST.GRID_TYPES.HEXEVENQ].includes(
            canvas.grid.type
          )
        ) {
      ...
      if (objData.width && objData.height && objData.width != null) {
        ...
        if (TokenAttacher.isGridSpace(type))
          [width, height] = [width * grid.sizeX, height * grid.sizeY];
        center = { x: x + Math.abs(width) / 2, y: y + Math.abs(height) / 2 };
      }
      //Drawings
      if (
        objData.shape?.width &&
        objData.shape?.height &&
        objData.shape?.width != null
      ) {
      ...
      if ("c" in objData) {
        center = {
          x: (objData.c[0] + objData.c[2]) / 2,
          y: (objData.c[1] + objData.c[3]) / 2,
        };
      }
      //TODO: Add hook
      return center;
    ...
    static async visualizeVector(x1, y1, x2, y2) {
      ...
      return await TokenAttacher.safeCreateEmbeddedDocuments(
        "Drawing",
        [drawingData],
        options
      );
    ...
    static getSize(objData) {
      return [
        objData.width ??
          objData.radius ??
          objData.distance ??
          (objData.config?.dim > objData.config?.bright
            ? objData.config?.dim
            : objData.config?.bright) ??
    ...
    static zSort(up, type, elements_data) {
      ...
      if (up) {
        ...
        z_background = siblings.length
          ? Math.max(...siblings.map((o) => o.document.sort)) + 1
          : 1;
        z_foreround = overhead_siblings.length
          ? Math.max(...overhead_siblings.map((o) => o.document.sort)) + 1
          : 1;
      } else {
        elements_data.sort((a, b) => b.sort - a.sort);
        z_background = siblings.length
          ? Math.min(...siblings.map((o) => o.document.sort)) - 1
          : -1;
        z_foreround = overhead_siblings.length
          ? Math.max(...overhead_siblings.map((o) => o.document.sort)) + 1
          : 1;
    ...
    static async setQuickEditMode(value) {
      ...
      if (value) {
        ...
        if (foundry.utils.getProperty(window, "tokenAttacher.quickEdit")) {
          ...
          const quickEdit = foundry.utils.duplicate(
            window.tokenAttacher.quickEdit
          );
    ...
    static updateOffset(type, document, change, options, userId) {
      ...
      let offset = foundry.utils.getProperty(
        document,
        `flags.${moduleName}.offset`
      );
      if (!offset) return;
      ...
      if (game.userId === userId && game.user.isGM) {
        ...
        if (quickEdit && canvas.scene._id === quickEdit.scene) {
          ...
          const parent_id = foundry.utils.getProperty(
            document,
            `flags.${moduleName}.parent`
          );
          let parent = TokenAttacher.layerGetElement(parent_type, parent_id);
          TokenAttacher.updateOffsetOfElement(
            quickEdit,
            parent_type,
            parent.document,
            type,
    ...
    static updateOffsetOfElement(
      ...
      if (!foundry.utils.getProperty(quickEdit, `elements.${type}`))
        quickEdit.elements[type] = [];
      const elemIndex = quickEdit.elements[type].findIndex(
        (item) => item._id === element_id
      );
      if (elemIndex === -1)
        quickEdit.elements[type].push({ _id: element_id, offset: new_offset });
    ...
    static canvasInit(canvasObj) {
      if (game.user.isGM) {
        ...
        if (
          quickEdit &&
          canvasObj.scene._id !== quickEdit.scene &&
          quickEdit.timer !== null
        ) {
          ui.notifications.error(
            game.i18n.format(localizedStrings.error.QuickEditNotFinished)
          );
        }
      }
    }
    ...
    static getLayerOrCollection(key) {
      return (
        canvas.getLayerByEmbeddedName(key) ??
        TokenAttacher.getLayerByEmbeddedName(key) ??
        game.collections.get(key)
      );
    }

    static async setAnimateStatus(
      tokens,
      animate,
      suppressNotification = false
    ) {
      let updates = tokens.map((elem) => {
        return {
          _id: elem.document._id,
          [`flags.${moduleName}.animate`]: animate,
        };
      });
      await TokenAttacher.safeUpdateEmbeddedDocuments(
        tokens[0].layer.constructor.documentName,
        updates,
        { [moduleName]: { update: true } }
      );
      if (!suppressNotification)
        ui.notifications.info(
          game.i18n.format(localizedStrings.info.AnimationToggled, {
            count: tokens.length,
          })
        );
    }
    ...
    static async toggleAnimateStatus(tokens, suppressNotification = false) {
      let updates = tokens.map((elem) => {
        return {
          _id: elem.document._id,
          [`flags.${moduleName}.animate`]: !(
            elem.document.getFlag(moduleName, `animate`) ?? true
          ),
        };
      });
      await TokenAttacher.safeUpdateEmbeddedDocuments(
    ...
    static layerGetElement(layer, id) {
      try {
        const layerObj =
          canvas.getLayerByEmbeddedName(layer) ??
          TokenAttacher.getLayerByEmbeddedName(layer);
        const foreground = canvas.foreground ?? layerObj;
        let result = { element: null };
        if (layerObj) result.element = layerObj.get(id) ?? foreground?.get(id);
        if (!result.element)
          Hooks.callAll(`${moduleName}.layerGetElement`, layer, id, result);
        return result.element;
      } catch (err) {
        console.warn(
          `Token Attacher: Error getting element ${id} from layer ${layer}`,
          err
        );
        return null;
      }
    ...
    static async migrateElementsInCompendiums(
      migrateFunc,
      elementTypes,
      topLevelOnly
    ) {
      ...
      for (let i = 0; i < allCompendiums.length; i++) {
        ...
        for (const index of packIndex) {
          ...
          if (prototypeAttached) {
                ...
                if (children.hasOwnProperty(key)) {
                  for (let i = 0; i < children[key].length; i++) {
                    ...
                    if (
                      ...
                      console.error(
                        `Token Attacher - Migration Error, attached child is not an object. Base Token and Actor: `,
                        base_entity.name,
                        base,
                        base_entity
                      );
            ...
            let new_token = foundry.utils.duplicate(
              foundry.utils.getProperty(entity, `prototypeToken`)
            );
            if (elementTypes.includes("Token"))
              updateElement(new_token, "Token", entity);
        ...
        console.log(
          "Token Attacher - Done migrating Elements in " + pack.metadata.label
        );
      }
      console.log(
        `Token Attacher - Done migrating ${elementCount} Elements in ${allCompendiums.length} Compendiums!`
      );
    }

    static async migrateElementsOfActor(
      actor,
      migrateFunc,
      elementTypes,
      topLevelOnly,
      ...
      if (prototypeAttached) {
        const new_token = TokenAttacher.migratePrototype(
          foundry.utils.getProperty(actor, `prototypeToken`),
          migrateFunc,
          elementTypes,
          topLevelOnly,
    ...
    static async migratePrototype(
      ...
      if (prototypeAttached) {
          ...
          for (const key in children) {
            if (children.hasOwnProperty(key)) {
              for (let i = 0; i < children[key].length; i++) {
                ...
                if (typeof element === "string" || element instanceof String) {
                  console.error(
                    `Token Attacher - Migration Error, attached child is not an object. Base and top level Base: `,
                    base,
                    base_entity
                  );
                  continue;
        ...
        if (elementTypes.includes("Token"))
          updateElement(new_token, "Token", prototypeToken);
      ...
      for (const type of TokenAttacher.registeredLayers) {
        ...
        const deleteLinks = (layer) => {
          ...
          for (let i = 0; i < layer.placeables.length; i++) {
            const element = layer.placeables[i];
            if (
              foundry.utils.getProperty(element.document, `flags.${moduleName}`)
            )
              pushUpdate(
                type,
                { _id: element.document._id, [`flags.-=${moduleName}`]: null },
                updates
              );
          }
        };
    ...
    static async migrateAttachedOfBase(
      base,
      migrateFunc,
      elementTypes,
      topLevelOnly,
      ...
      return TokenAttacher.migrateAttached(
        base.layer.constructor.documentName,
        base,
        migrateFunc,
        elementTypes,
        topLevelOnly,
        ...
        if (attachedEntities.hasOwnProperty(key)) {
          for (let i = 0; i < attachedEntities[key].length; i++) {
            ...
            if (Object.keys(elem_attached).length > 0) {
              ...
              const subUpdates = await TokenAttacher.migrateAttached(
                key,
                updatedElementData,
                migrateFunc,
                elementTypes,
    ...
    static InstantAttach(type, document, options, userId) {
      if (!foundry.utils.getProperty(options, `${moduleName}.InstantAttach`))
        return;
      if (
        !foundry.utils.getProperty(
          options,
      ...
      let element;
      if (attachment_base) {
        element = TokenAttacher.layerGetElement(
          attach_base.type,
          attach_base.element
        );
      }
      ...
      TokenAttacher._AttachToToken(element, {
        type: document.layer.constructor.documentName,
        ids: [document._id],
      });
    }
    ...
    static getLayerByEmbeddedName(type) {
      try {
        ...
        console.warn(
          `Token Attacher: Could not find layer for type ${type}`,
          err
        );
        return null;
      }
    ...
    static async validateDocument(document, operation = "update") {
      if (!document || !document._id) {
        console.warn(
          `Token Attacher: Invalid document for ${operation} operation`
        );
        return false;
      }
      return true;
    ...
    static async safeUpdateEmbeddedDocuments(type, updates, options = {}) {
      try {
        // Filter out invalid updates
        const validUpdates = updates.filter((update) => update && update._id);
        if (validUpdates.length === 0) return;

        return await TokenAttacher.safeUpdateEmbeddedDocuments(
          type,
          validUpdates,
          options
        );
      } catch (err) {
        console.error(`Token Attacher: Error updating ${type} documents`, err);
        ui.notifications.error(
          `Failed to update ${type} documents. See console for details.`
        );
      }
    }
    ...
    static async renderTemplate(templatePath, data = {}) {
      try {
        ...
        console.error(
          `Token Attacher: Template rendering failed for ${templatePath}`,
          err
        );
        ui.notifications.error(
          "Template rendering failed. See console for details."
        );
        return `<div>Error rendering template: ${templatePath}</div>`;
      }
    }
  }
