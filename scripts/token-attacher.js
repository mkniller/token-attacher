...
(async () => {
	...
	class TokenAttacher {
		...
		static async _AttachToToken(token, elements, suppressNotification = false, return_data = false) {
			...
			if (typeof token === 'string' || token instanceof String) token = canvas.tokens.placeables.find(t => t.id === token);
		...
		static async _attachElementsToToken(selected, target_token, suppressNotification = false) {
			if (typeof target_token === 'string' || target_token instanceof String) target_token = canvas.tokens.placeables.find(t => t.id === target_token);
			let updates = {};
		...
		*/
		...
		static async updateAttachedCreatedToken(type, document, options, userId) {
			...
			if (!TokenAttacher.isFirstActiveGM()) return;
			const token = canvas.tokens.placeables.find(t => t.id === document._id);
			if (!token) return;
		...
		*/
		...
		static async exportCompendiumToJSON(pack) {
			...
			const html = await renderTemplate(`${templatePath}/ImExportUI.html`, {
				label_content: "Copy the JSON below:", content: JSON.stringify({
					compendium: {
		...
		static isAllowedToMove(type, document, change, options, userId) {
			...
			if (!foundry.utils.getProperty(options, `${moduleName}.update`)
				...
				const parent_token = canvas.tokens.placeables.find(t => t.id === objParent);
		...
		static isCurrentAttachUITarget(id) {
			const elementId = canvas.scene.getFlag(moduleName, "attach_base")?.element;
			const token = canvas.tokens.placeables.find(t => t.id === elementId);
			return token?.document._id === id;
		}
		...
		static async _RectangleSelection(event) {
			...
			const token = canvas.tokens.placeables.find(t => t.id === baseId);
			for (const type of TokenAttacher.registeredLayers) {
				const layer = canvas.getLayerByEmbeddedName(type) ?? TokenAttacher.getLayerByEmbeddedName(type);
